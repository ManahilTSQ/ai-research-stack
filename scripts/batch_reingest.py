...
def run():
    ...
    for pdf_path in pdf_files:
        ...
        try:
            ...
            if ok:
                manifest_svc.mark_as_ingested(filename, title, doi, status="success", has_full_text=len(full_text) > 8000)
                print(f"  [OK] SUCCESS - '{title}' ingested with {len(chunks)} chunks.")
                success += 1
            else:
                manifest_svc.mark_as_ingested(filename, title, doi, status="failed",
                                              error="ChromaDB upsert returned False.", has_full_text=len(full_text) > 8000)
